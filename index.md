---
layout: default
title: MIT OpenCourseWare - Compiler Engineering
---

# MIT OpenCourseWare - Compiler Engineering Series

**By Harris Maung**

*Based on MIT 6.035 - Computer Language Engineering*

---

## 👋 Welcome!

I'm documenting my journey learning compiler design through MIT's Computer Language Engineering course. These aren't just notes—they're readable, digestible articles with examples, quizzes, and practice problems!

Each lecture includes:
- 📖 Clear explanations with real-world examples
- 🎨 Diagrams and visualizations
- 💻 Code walkthroughs and optimizations
- ✅ Quiz questions to test understanding
- 🏋️ Practice problems with detailed solutions

---

## 📚 Lecture Series

### [Lecture 1: Introduction to Compilers](learning/MIT_opencourseware/lecture_1_intro/data.md)

**Topics covered:**
- Why study compilers?
- Programming language requirements (precision, expressiveness, abstraction)
- Anatomy of a compiler pipeline (Lexer → Parser → AST → IR → Optimizer → Code Gen)
- What are AST and IR? (with detailed examples)
- 8 compiler optimization techniques:
  - Constant propagation & folding
  - Dead code elimination
  - Common subexpression elimination
  - Loop invariant code motion
  - Strength reduction (bit shifts)
  - Induction variable optimization
- Assembly code comparison showing **4x performance improvement**
- Quiz questions + 3 practice problems with solutions

[**📖 Read Lecture 1 →**](learning/MIT_opencourseware/lecture_1_intro/data.md)

---

### Lecture 2: Lexical Analysis *(Coming soon...)*

**Topics:** Tokenization, regular expressions, finite automata, scanner implementation

---

### Lecture 3: Parsing *(Coming soon...)*

**Topics:** Context-free grammars, top-down vs bottom-up parsing, AST construction

---

### Lecture 4: Semantic Analysis *(Coming soon...)*

**Topics:** Type checking, symbol tables, scope resolution

---

## 📖 Additional Resources

- [MIT 6.035 OpenCourseWare](https://ocw.mit.edu/courses/6-035-computer-language-engineering-spring-2010/)
- [Dragon Book - Compilers: Principles, Techniques, and Tools](https://en.wikipedia.org/wiki/Compilers:_Principles,_Techniques,_and_Tools)
- [Crafting Interpreters by Robert Nystrom](https://craftinginterpreters.com/)
- [LLVM Documentation](https://llvm.org/docs/)

---

## 🤝 About This Project

This is a learning-in-public project! If you find it helpful:
- ⭐ Star the [GitHub repo](https://github.com/yourusername/rustphy)
- 📢 Share with others learning compilers
- 💬 Open an issue with questions or feedback
- 🐛 Found a mistake? Submit a PR!

---

## How to Use This Series

1. **Read sequentially** - Each lecture builds on previous concepts
2. **Try the quizzes** - Test your understanding before moving on
3. **Do the practice problems** - Hands-on coding solidifies learning
4. **Take your time** - Compilers are complex; don't rush

---

*Learning in public, one lecture at a time* ✨

**Author:** Harris Maung  
**Source:** MIT 6.035 - Computer Language Engineering
